...
def create_mcp(mcp: MCPServer):
    ...
    try:
        ...
        if mcp.github_link:
            ...
            print(f"Tools 추출 조건 확인: github_link={bool(mcp.github_link)}, tools_empty={not mcp.tools}")
        
        # GitHub 링크에서 resources 정보 자동 추출 (항상 추출)
        if mcp.github_link:
            print(f"GitHub 링크에서 resources 정보 추출 중: {mcp.github_link}")
            extracted_resources = mcp_service.read_mcp_server_resource_list(mcp.github_link)
