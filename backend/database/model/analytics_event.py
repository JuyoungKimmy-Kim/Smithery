...
class AnalyticsAggregation(Base):
    """
    집계된 분석 데이터를 미리 계산하여 저장

    대량의 이벤트 데이터를 매번 집계하는 것은 비효율적이므로,
    일별/시간별로 미리 집계한 데이터를 저장합니다.

    예시:
    - aggregation_type = "daily_server_views"
    - aggregation_key = "server_123"
    - aggregation_value = 1500 (하루 조회수)
    - aggregation_date = "2025-01-24"
    """
    __tablename__ = "analytics_aggregations"

    id = Column(Integer, primary_key=True, index=True)

    # 집계 타입 (예: "daily_server_views", "hourly_searches", "weekly_top_keywords")
    aggregation_type = Column(String(100), nullable=False, index=True)

    # 집계 키 (예: "server_123", "keyword_weather", "user_456")
    aggregation_key = Column(String(255), nullable=False, index=True)

    # 집계 값 (숫자형)
    aggregation_value = Column(Integer, default=0, nullable=False)
    ...
    # 복합 인덱스 (집계 조회 성능 최적화)
    __table_args__ = (
        Index('ix_agg_type_date', 'aggregation_type', 'aggregation_date'),
        Index('ix_agg_type_key_date', 'aggregation_type', 'aggregation_key', 'aggregation_date'),
    )
    ...
    def __repr__(self):
        return f"<AnalyticsAggregation(type={self.aggregation_type}, key={self.aggregation_key}, value={self.aggregation_value}, date={self.aggregation_date})>"
