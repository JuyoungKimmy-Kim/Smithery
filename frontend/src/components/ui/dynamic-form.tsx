  ...
  const switchToFormMode = () => {
    ...
    if (jsonText.trim()) {
      try {
        ...
        setJsonError('Please fix JSON errors before switching to form mode');
        return;
      }
    } else {
      setIsJsonMode(false);
      ...
      return (
        <div key={fullPath} className="mb-6">
          <label className="block text-sm font-medium text-gray-700 mb-1">
            {key}
            {(path ? getNestedValue(jsonSchema, path)?.required?.includes(key) : jsonSchema.required?.includes(key)) && <span className="text-red-600 ml-1">*</span>}
          </label>
                    ...
                    {Object.entries(propSchema.items.properties).map(([objKey, objSchema]) => (
                      <div key={objKey}>
                        <label className="block text-xs font-medium text-gray-600 mb-1">
                          {objKey}
                          {propSchema.items?.required?.includes(objKey) && <span className="text-red-600 ml-1">*</span>}
                        </label>
      ...
      if (propSchema.enum) {
        return (
          <div key={fullPath} className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {key}
              {(path ? false : jsonSchema.required?.includes(key)) && <span className="text-red-600 ml-1">*</span>}
            </label>
            {propSchema.description && (
              <p className="text-xs text-gray-500 mb-2">{propSchema.description}</p>
            )}
    ...
    return (
      <div key={fullPath} className="mb-4">
        ...
        <input
          ...
          value={value || ''}
          onChange={(e) => handleInputChange(fullPath, e.target.value)}
          placeholder={`Enter ${propSchema.type} value`}
          className={`w-full border rounded-md px-3 py-2 ${error ? 'border-red-500' : 'border-gray-300'} focus:outline-none focus:ring-2 focus:ring-blue-500`}
        />
        {error && <p className="text-red-600 text-xs mt-1">{error}</p>}
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center pb-3">
        <h6 className="text-md font-medium text-gray-900">{title || 'Parameters'}</h6>
